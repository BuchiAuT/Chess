...
public class SpringerTest 
{
	...
	{		
		try
		{
			...
			boolean beobachtet =könig.spielzugMoeglich(sf, new Position(2,1));
			Assert.assertTrue(beobachtet);
			
			// 2 nach rechts auf 1 nach oben freies Feld (5,4)
			beobachtet=könig.spielzugMoeglich(sf, new Position(5,4));
			Assert.assertTrue(beobachtet);
			
			// 2 nach links 1 nach unten auf freies Feld (1,2)
			beobachtet=könig.spielzugMoeglich(sf, new Position(1,2));
			Assert.assertTrue(beobachtet);
			
			// 2 nach oben 1 nach rechts (4,5)
			beobachtet=könig.spielzugMoeglich(sf, new Position(4,5));
			Assert.assertTrue(beobachtet); 
